 ...
 -->
	...
	<div class="container">
		<div class="row">
			...
			<div class="col-sm-4">
				<h3 class="text-center">Usuario Administrativo</h3>
  ...
  ?>
  <form method="POST" action="upload.php" enctype="multipart/form-data" class="col-sm-12 justify-content-center">
    <div  class="col-sm-12 moto">
      <span>Subir archivo de cruce de operaciones:</span>
      <input type="file" name="uploadedFile" class="btn btn-warning" />
    <input type="submit" name="uploadBtn" value="Cargar Archivo Excel" class="btn btn-primary" />
    <input  type="hidden" name="idCruce" value="1"/>
    </div>
  </form>
		
		<form action="<?php echo $helper->url("BandejaBancos","procesarPagos"); ?>" method="post" class="col-sm-12 justify-content-center">
        <section class="col-sm-12 moto">
            <?php 
           // if(isset($_GET["vca"]) && $_GET["vca"]==1)
            //{
                echo '<input type="submit" value="Cruzar Operaciones" class="btn btn-danger" name="registrar"/>';
            //}
            //else
            //{
              //  echo '<input type="submit" value="Cruzar OperacionesX" class="btn btn-danger" name="registrar"/>';
            //}
        ...
        <section class="col-sm-12 moto">
            <?php 
             ...
             if (isset($allMisAportes) && count($allMisAportes)>=1)
             {
				...
				$display="block";
				$dir =  $helper->url("BandejaBancos","index");
            ...
            <?php 
             ...
             if (isset($allMisAportes2) && count($allMisAportes2)>=1)
             {
                ...
                foreach($allMisAportes2 as $aporte) {
                    ...
                    <td class="right">
                    <a name="editar" href="'.$dir.'&idx='.$aporte->aporteID.'&datosx='.$aporte->value.' - '.$aporte->bank.' - '.$aporte->account.'" style="display:'.$visValidar.'" class="btn btn-primary">Validar Aporte</a>
		...
		<form action="<?php echo $helper->url("BandejaBancos","index"); ?>" method="post" class="col-sm-12 justify-content-center">
        <section class="col-sm-12 moto">
            <?php 
             //Carga el array que contiene MisCitas en una tabla siempre y cuando haya más de un registro
