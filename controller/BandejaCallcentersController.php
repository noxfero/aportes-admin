...
*/
class BandejaCallcentersController extends ControladorBase{
    public $conectar;
	public $adapter;
    ...
    public function index(){
		...
		$allMisAportes3=$payment->getAllofOneConEstadoLimit('"isActive"','1','"isMatched"','0','"registeredDate"',$offset3);	
         ...
         $this->view("bandejaCallcenter",array(
    ...
    public function agregaAportes(){
        ...
        if(isset($_POST["aportanteID"]) && !empty($_POST["aportanteID"])){ // Si hay datos
            if(isset($_POST["value"]) && isset($_POST["account"]) && isset($_POST["bank"])){
                ...
                if ($save == TRUE)
                {
                    $aporte->phpAlert("Aporte creado con éxito",$this->baseUrl("BandejaCallcenters", "index")); // Alerta y redirige
                }
                else
                {
                    $aporte->phpAlert("Error con el registro. Interente nuevamente",$this->baseUrl("BandejaCallcenters", "index")); // Alerta y redirige
        ...
        {
            $aporte->phpAlert("Debe seleccionar priemro un aportante para poder guardar el aporte.",$this->baseUrl("BandejaCallcenters", "index")); // Alerta y redirige
        }
    ...
    public function enviarCorreo(){
        ...
        if(isset($_POST["aporteIDx"])){ // Si hay datos
            if(isset($_POST["isActivex"]) && isset($_POST["aportanteIDx"])){
                ...
                if ($save == TRUE)
                {
                    $aporte->phpAlert("Correo enviado éxito a ".$_POST["isActivex"] ,$this->baseUrl("BandejaCallcenters", "index")); // Alerta y redirige
                    $aporte=new Aporte($this->adapter);
                ...
                {
                    $aporte->phpAlert("Error con el envío de correo. Interente nuevamente",$this->baseUrl("BandejaCallcenters", "index")); // Alerta y redirige
        ...
        {
            $aporte->phpAlert("Seleccione datos.",$this->baseUrl("BandejaCallcenters", "index")); // Alerta y redirige
        }
    ...
    public function borrarMoto(){
        ...
        $this->redirect("BandejaCallcenters", "index");
    }
